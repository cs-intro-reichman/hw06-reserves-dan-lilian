...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		print(tinypic2);
		System.out.println();
		print(blend(tinypic, tinypic2, 0.5));
	...
	public static Color[][] read(String fileName) {
		...
		for (int i=0; i <image.length;i++)
		{
			...
			{
				Color tempPixel = new Color (in.readInt(),in.readInt(),in.readInt());
				image[i][j]= tempPixel;
			}
		}
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		{
			int index = 0;
			for(int j=(flippedImage[i].length-1);j>=0;j--)
			{
				flippedImage[i][j]= image[i][index];
				index++;
			}
		}
		return flippedImage;
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for(int i=(flippedImage.length-1); i>=0;i--)
		{
			...
			{
				flippedImage[i][j]= image[index][j];
			}
			index++;
		}
		return flippedImage;
	...
	public static Color[][] grayScaled(Color[][] image) {
		...
		{
			for(int j=0;j<GSImage[i].length; j++)
			{
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		{
			for( int a=0; a<scaledImage[0].length;a++)
			{
				int y = (int) (i * hRatio);
                int x = (int) (a * wRatio);
				scaledImage[i][a]=image[y][x];
			}
		}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		int g2= c2.getGreen();
		int b2= c2.getBlue();
		int nr= (int)(alpha*r1 + (1-alpha)*r2);
		int ng= (int)(alpha*g1 + (1-alpha)*g2);
		int nb= (int)(alpha*b1 + (1-alpha)*b2);
		Color blended= new Color(nr,ng,nb);
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		if((source.length!=target.length)||(source[0].length!=target[0].length))
		{
			...
			for(int i=0;i<=n;i++)
		    {
			  ...
			  blended=blend(source, newtarget, a);
			  display(blended);
			  StdDraw.pause(500);
		    }
		}else{
		...
		{
			Color[][] blended = new Color[source.length][source[0].length];
			double a = (double)((n-j)/n);
			blended=blend(source, target, a);
			display(blended);
			StdDraw.pause(500);
		}
